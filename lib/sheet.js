...
Builder.prototype.output = function (cb) {
    try {
        cb(null, autoprefixer.process(this.rework.toString()).css);
...
var bootstrapGrid = require("./bootstrap-grid.js");
Builder.prototype.useBootstrapGrid = function () {
    try { this.rework = this.rework.use(bootstrapGrid); }
    catch (err) { this.onError(err, "bootstrap-grid"); }
    return this;
};

var reworkColor = require("rework-color-function");
Builder.prototype.useColor = function(){
    try {
        this.rework = this.rework.use(reworkColor);
    } catch(err){
        this.onError(err, "rework-color");
    }
    return this;
};
